function [optError,fullError,overlapError, figHandle] = MatchingResult(i,gtFiles,resPath,I1,I2,a,optA,prefixName,templateMask)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%plot([cornerAxs cornerAxs(1)],[cornerAys cornerAys(1)], '-k');

%Frame=getframe;

fill([cornerAxs cornerAxs(1)],[cornerAys cornerAys(1)],'w');
Frame=getframe;

resName = sprintf([gtFiles(i).name(1:end)  ]);
%resName = sprintf('%d.jpg',i);
imwrite(Frame.cdata,fullfile(resPath,resName));

%a=imread(fullfile(resPath,resName));
%b=im2bw(a); 
%imwrite(b,fullfile(resPath,resName)) ;



%Frame=getframe;
%imwrite(Frame.cdata,'image/11111.jpg');

%imwrite([cornerAxs cornerAxs(1)],[cornerAys cornerAys(1)],'image/11111.jpg');
...
%imwrite(),'image/11111.jpg');


%imshow(I2);
%imshow(I2); 
%hold on;
%plot([cornerAxs cornerAxs(1)],[cornerAys cornerAys(1)], '-r');

%Frame=getframe;
%imwrite(Frame.cdata,'image1/1111111.jpg') ;
